...
path_out <- here("3-outputs", "models", "models-case-crossover")
!dir.exists(path_out) && dir.create(path_out, recursive = TRUE)

# Load models ----
path_processed <- here("2-data", "2.2-processed-data")
model_outputs_pnc_b <- readRDS(here(path_processed, "2.5-models-unwt-case-crossover-pnc-b-psu.rds"))
model_outputs_pnc_m <- readRDS(here(path_processed, "2.5-models-unwt-case-crossover-pnc-m-psu.rds"))
model_outputs_s2s <- readRDS(here(path_processed, "2.5-models-unwt-case-crossover-s2s-psu.rds"))
print("finished loading models")
names(model_outputs_s2s)
...
all_models <- list(model_outputs_pnc_b, model_outputs_pnc_m, model_outputs_s2s)

# Initialize an empty list to store tidy outputs
tidy_outputs <- list()

for(model_outputs in all_models) {
...
saveWorkbook(wb, here(path_out, "models_case_crossover_unwt_psu.xlsx"), overwrite = TRUE)
...
for(model_name in names(tidy_outputs)) {
  ...
  second_row <- cbind(dep_var, exp, estimates)
...
write.csv(combined_exposures, here(path_out, "models_case_crossover_consolidated_unwt_psu.csv"), row.names = FALSE)
