...
df0_add <- df0 %>% 
    mutate(comp_vision = m47) %>% 
    mutate(comp_convulsions = ifelse(SurveyId == "IA2015DHS", s432, s434)) %>%
    mutate(comp_swelling = ifelse(SurveyId == "IA2015DHS", s433, s435)) %>%
...
tabyl(df_merge_proc, dv_comp_vision_bi)
tabyl(df_merge_proc, dv_comp_convulsions_bi)
tabyl(df_merge_proc, dv_comp_swelling_bi)
